    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-8 text-center">
          <p className="text-gray-500">No {filter !== 'all' ? filter : ''} items in the queue.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {items.map((item) => (
            <div key={item._id} className="bg-gray-900 border border-gray-800 rounded-xl p-5">
              <div className="flex items-start justify-between mb-3">
                <div>
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-medium bg-gray-800 text-gray-300 px-2 py-0.5 rounded-full uppercase">
                      {item.type}
                    </span>
                  ...
                  <p className="text-sm text-gray-300 mt-1">
                    By <span className="font-medium text-white">{item.userName}</span>
                  </p>
                </div>
              ...
              <p className="text-sm text-gray-400 bg-gray-800/50 rounded-lg p-3 line-clamp-3">{item.content}</p>
